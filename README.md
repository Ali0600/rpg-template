...
`tools/check.sh` runs lint, parse, compile, 1,590 tests, a boot check, an artifact drift check
...
![The hollow's slink pair, fought with the sword - the lit block is the blade's reach](docs/images/arena.png)
...
- Designed an asset pipeline with the same output every time: procedural sprites drawn from
  ...
  output drifts.
- Built a CI/CD pipeline in GitHub Actions that fails closed: lint → parse → compile → 1,590
...
- Added mutation testing over the project's own quality gates: 827 mutants, each proving a rule
