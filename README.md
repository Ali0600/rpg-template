...
`tools/check.sh` runs lint, parse, compile, 1,718 tests, a boot check, an artifact drift check
...
- Designed an asset pipeline with the same output every time: procedural sprites drawn from
  ...
  output drifts.
- Built a CI/CD pipeline in GitHub Actions that fails closed: lint → parse → compile → 1,718
...
- Added mutation testing over the project's own quality gates: 966 mutants, each proving a rule
