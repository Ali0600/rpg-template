...
func before_test() -> void:
	GameState.reset()
	Router.reset()
	# Without this the suite writes into the real user://saves/demo/ - a developer's own
...
## A perfectly good demo save, built without going through the live state.
func _demo_save() -> SaveData:
...
func _boot() -> Node2D:
	...
	add_child(_world)
	assert_bool(_world.start_game(load(DEMO) as GameManifest)).override_failure_message(
		"the world would not start the demo").is_true()
	return _world
...
func test_saving_from_the_menu_writes_this_games_slot() -> void:
	...
	var written := SaveManager.peek(&"demo", 0)
	assert_object(written).is_not_null()
	assert_str(String(written.game)).is_equal("demo")
	assert_str(String(written.map)).is_equal("demo_town")
...
func test_a_save_made_in_another_map_restores_into_that_map() -> void:
	# A save records a map as well as a position, and the demo has two. Restoring into the one
	...
	assert_str(String(_world.map_data().id)).is_equal("demo_cave")
	assert_str(String(GameState.current_map)).is_equal("demo_cave")
	assert_str(Router.state_name()).is_equal("world")

func test_another_games_save_is_never_even_offered() -> void:
	# Fail closed at the first opportunity: a quest save sitting in the demo's slot does not
	...
	_boot()
	var quest := SaveData.new()
	quest.game = &"quest"
	quest.map = &"quest_village"
	quest.position = Vector2(64.0, 64.0)
	assert_bool(SaveManager.save(2, quest)).is_true()
	SaveDirs.write_raw(&"demo", 2, FileAccess.get_file_as_string(SaveManager.slot_path(&"quest", 2)))
	...
	assert_str(String(_world.map_data().id)).is_equal("demo_town")
	assert_bool(FileAccess.file_exists(SaveManager.corrupt_path(&"demo", 2))).override_failure_message(
...
func test_a_slot_that_goes_bad_while_the_menu_is_open_is_refused_and_the_menu_lives() -> void:
	...
	_boot()
	assert_bool(SaveManager.save(2, _demo_save())).is_true()

	assert_bool(_world.open_pause()).is_true()
	await _to_the_third_slot()
	# The file changes under the open menu.
	SaveDirs.write_raw(&"demo", 2, "{ truncated half way throu")
	await _press(&"interact")
	...
	assert_str(FileAccess.get_file_as_string(SaveManager.corrupt_path(&"demo", 2))).override_failure_message(
...
func test_switching_games_closes_the_pause_menu() -> void:
	# The menu lists ONE game's slots. Left standing over another game it would offer to load
	# saves that cannot load, and to write slots belonging to a game nobody is playing.
	_boot()
	_world.open_pause()
	var screen: PauseScreen = _world.pause_screen()
	_world.start_game(load("res://data/games/quest.tres") as GameManifest)
	assert_bool(is_instance_valid(screen)).override_failure_message(
