...
- [x] **M40a** — hand-drawn characters: a style's sheets come from the rig or from an import (`sheets_from`), Universal LPC exports are converted at build time, licence-gated by file, credited beside the sprites and drift-gated like everything else. The world at 32px and the cast itself follow as M40b and M40c; the credits screen was re-lettered past and landed as M43
...
- [x] **M42** — the interface rebuilt: one pixel font, framed windows, a cursor bar, coloured HP/MP, and a face in the fight, the menu and the conversation
- [x] **M43** — the art has a credits screen, which the licence has owed since M40: a Credits row on the title opens the composed attribution list the generator already wrote into the pack — a notice of what the art is and whose terms it is under, then all 43 artists by name, paged and gated so no name can be trimmed
- [x] **M44** — `GameConfig` states distances in TILES: the demo's six doubled pixel numbers are gone and it inherits the template's, because 3 tiles a second is 3 tiles a second at any art size. `at(tile_size)` binds a duplicate and the `_px()` accessors are the only way to a pixel, so `Locomotion`, `GridWalker`, `Interactor` and `ActorBody` stay pure; `grid_step` became a bool, which made "a step that is not a tile" unrepresentable and retired the manifest cross-check that used to refuse it
- [x] **M45** — the game stopped running a platformer's rulebook: `ActorBody` sets `MOTION_MODE_FLOATING`, so every contact is a wall and the compass is no longer mistaken for gravity. The `platform_floor_layers` opt-out M13.4 added went with it — under floating it is a guard no test could watch fail. Re-measured against 24 sessions rather than the eleven the deferral was argued on: 23 passed untouched, and the one that did not was a leg the contract already forbade, re-anchored so it now passes under both motion modes
- [x] **M46** — an Options screen, and the windows are the player's to recolour: a `UiPalette` under `data/palettes/` is a whole set of the eight interface roles, laid over the running style by `_bind_style` and kept beside the volume in the player's own settings file. It is reached from a row on the title and a row in the pause menu — one screen, and the menu closes on the way through because no overlay here opens over another. The pause menu's Sound row became Options and stopped carrying a word. The references were read from their disassemblies first, and they disagree — Final Fantasy VI gives three colour bars, Pokémon Gen I gives nothing and Gen II a numbered border style — so the divergence is stated: eight roles move together or `text` stops being legible on `panel`
