...
- [x] **M47** — a new game is a command, and two gates stopped assuming there is only one: `tools/new_game.sh --id=<name>` writes a manifest, a walled first room with a greeter in it, that greeter's line, an optional config, combat definition and hooks file, and a scripted play session — so the new game is inside the play gate the day it is made. Everything it decides is a pure function tested with no disk, and an integration gate boots the generated game and walks the player in it, which is the reusability claim M7 proved by hand and M11 retired. Finding it meant fixing the two content gates that hardcoded one game: the map walk loaded the demo's manifest by literal path, and the portrait check crossed every game with every conversation in the repo
