...
- [x] **M46** — the windows are the player's to recolour: a `UiPalette` under `data/palettes/` is a whole set of the eight interface roles, laid over the running style by `_bind_style` and kept beside the volume in the player's own settings file. The references were read from their disassemblies first, and they disagree — Final Fantasy VI gives three colour bars, Pokémon Gen I gives nothing and Gen II a numbered border style — so the divergence is stated: eight roles move together or `text` stops being legible on `panel`
